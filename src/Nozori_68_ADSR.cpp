...
struct Nozori_68_ADSRWidget : ModuleWidget {
	...
	Nozori_68_ADSRWidget(Nozori_68_ADSR* module) {
		...
		panelStandard->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_68_ADSR.svg")));
		panelStandard->show();
		addChild(panelStandard);

		panelDark = new SvgPanel();
		panelDark->box.size = box.size;
		panelDark->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_68_ADSR_dark.svg")));
		panelDark->hide();
		addChild(panelDark);
	...
	void appendContextMenu(Menu* menu) override {
		Nozori_68_ADSR* module = dynamic_cast<Nozori_68_ADSR*>(this->module);
		assert(module);
