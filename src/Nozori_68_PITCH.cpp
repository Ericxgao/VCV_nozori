...
struct Nozori_68_PITCHWidget : ModuleWidget {
	...
	Nozori_68_PITCHWidget(Nozori_68_PITCH* module) {
		...
		panelStandard->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_68_PITCH.svg")));
		panelStandard->show();
		addChild(panelStandard);

		panelDark = new SvgPanel();
		panelDark->box.size = box.size;
		panelDark->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_68_PITCH_dark.svg")));
		panelDark->hide();
		addChild(panelDark);
	...
	void appendContextMenu(Menu* menu) override {
		Nozori_68_PITCH* module = dynamic_cast<Nozori_68_PITCH*>(this->module);
		assert(module);
