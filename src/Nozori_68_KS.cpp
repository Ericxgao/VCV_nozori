...
//        time_since_startup += 1/args.sampleRate;
        ...
        if (reduce_data_speed_index == 0) {
            KS_loop_(); // process data loop only at 1/4 the sampling rate in order to be more accurate with the hardware timing
...
struct Nozori_68_KSWidget : ModuleWidget {
	...
	Nozori_68_KSWidget(Nozori_68_KS* module) {
		...
		panelStandard->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_68_KS.svg")));
		panelStandard->show();
		addChild(panelStandard);

		panelDark = new SvgPanel();
		panelDark->box.size = box.size;
		panelDark->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_68_KS_dark.svg")));
		panelDark->hide();
		addChild(panelDark);
	...
	void appendContextMenu(Menu* menu) override {
		Nozori_68_KS* module = dynamic_cast<Nozori_68_KS*>(this->module);
		assert(module);
