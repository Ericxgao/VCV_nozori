...
struct Nozori_84_HARMONICS : Module {
	enum ParamIds {
		POT2_PARAM,
		POT1_PARAM,
		POT3_PARAM,
...
*/
	void process(const ProcessArgs& args) override {
        ...
        if (reduce_data_speed_index == 0) {
            VCO_Harmo_loop_(); // process data loop only at 1/4 the sampling rate in order to be more accurate with the hardware timing
...
struct Nozori_84_HARMONICSWidget : ModuleWidget {
	...
	Nozori_84_HARMONICSWidget(Nozori_84_HARMONICS* module) {
		...
		panelStandard->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_84_HARMONICS.svg")));
		panelStandard->show();
		addChild(panelStandard);

		panelDark = new SvgPanel();
		panelDark->box.size = box.size;
		panelDark->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_84_HARMONICS_dark.svg")));
		panelDark->hide();
		addChild(panelDark);
		...
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(49.3, 113.3)), module, Nozori_84_HARMONICS::OUT2_OUTPUT));
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(36.8, 113.3)), module, Nozori_84_HARMONICS::OUT1_OUTPUT));

		addChild(createLightCentered<MediumLight<YellowLight>>(mm2px(Vec(58.3, 41.5)), module, Nozori_84_HARMONICS::LED4_LIGHT));
		addChild(createLightCentered<MediumLight<YellowLight>>(mm2px(Vec(2.9, 41.5)), module, Nozori_84_HARMONICS::LED2_LIGHT));

		addParam(createParamCentered<NKK>(mm2px(Vec(30.6, 14.3)), module, Nozori_84_HARMONICS::SWITCH_PARAM));
	...
	void appendContextMenu(Menu* menu) override {
		Nozori_84_HARMONICS* module = dynamic_cast<Nozori_84_HARMONICS*>(this->module);
		assert(module);
...
Model* modelNozori_84_HARMONICS = createModel<Nozori_84_HARMONICS, Nozori_84_HARMONICSWidget>("Nozori_84_HARMONICS");
