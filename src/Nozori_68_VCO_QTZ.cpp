...
//        time_since_startup += 1/args.sampleRate;
        ...
        if (reduce_data_speed_index == 0) {
            VCO_Q_loop_(); // process data loop only at 1/4 the sampling rate in order to be more accurate with the hardware timing
...
struct Nozori_68_VCO_QTZWidget : ModuleWidget {
	...
	Nozori_68_VCO_QTZWidget(Nozori_68_VCO_QTZ* module) {
		...
		panelStandard->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_68_VCO_Qtz.svg")));
		panelStandard->show();
		addChild(panelStandard);

		panelDark = new SvgPanel();
		panelDark->box.size = box.size;
		panelDark->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_68_VCO_Qtz_dark.svg")));
		panelDark->hide();
		addChild(panelDark);
		...
		addParam(createParamCentered<NozoriKnob>(mm2px(Vec(46.1, 30.3)), module, Nozori_68_VCO_QTZ::POT2_PARAM));
		addParam(createParamCentered<NozoriKnob>(mm2px(Vec(15.1, 30.3)), module, Nozori_68_VCO_QTZ::POT1_PARAM));
		addParam(createParamCentered<NozoriKnob>(mm2px(Vec(15.1, 52.8)), module, Nozori_68_VCO_QTZ::POT3_PARAM));
		addParam(createParamCentered<NozoriKnob>(mm2px(Vec(46.1, 52.8)), module, Nozori_68_VCO_QTZ::POT4_PARAM));
		addParam(createParamCentered<NozoriKnob>(mm2px(Vec(15.1, 75.1)), module, Nozori_68_VCO_QTZ::POT5_PARAM));
		addParam(createParamCentered<NozoriKnob>(mm2px(Vec(46.1, 75.1)), module, Nozori_68_VCO_QTZ::POT6_PARAM));
	...
	void appendContextMenu(Menu* menu) override {
		Nozori_68_VCO_QTZ* module = dynamic_cast<Nozori_68_VCO_QTZ*>(this->module);
		assert(module);
