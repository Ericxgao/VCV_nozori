...
struct Nozori_84_8_BITSWidget : ModuleWidget {
	...
	Nozori_84_8_BITSWidget(Nozori_84_8_BITS* module) {
		...
		panelStandard->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_84_8BITS.svg")));
		panelStandard->show();
		addChild(panelStandard);

		panelDark = new SvgPanel();
		panelDark->box.size = box.size;
		panelDark->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/nozori_84_8BITS_dark.svg")));
		panelDark->hide();
		addChild(panelDark);
